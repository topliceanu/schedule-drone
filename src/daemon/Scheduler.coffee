...
conf = require '../conf'
db = require '../db'
exponentialBackoff = require '../util/exponentialBackoff'
call = require '../util/call'


# This class polls the mongodb collection to find the jobs which should be
# executed currently.
# Because mongodb's findAndModify only updates and returns atomically only
# one document, this class will continuously poll the database. If an available
# job is found, it is updated and executed, then the collection is polled again
# immediately. If, on the contrary, nothing is found, the polling interval
...
class Scheduler
    ...
    start: ->
        exponentialBackoff @pollDatabase, @options

    pollDatabase: =>
        startInterval = moment.utc().add('hours', -1).toDate()
        endInterval = moment.utc().toDate()
        query =
            status: conf.const.JOB_STATE.ENABLED
            start:
                $gte: startInterval
                $lte: endInterval
        updates =
            $set: {status: conf.const.JOB_STATE.DISABLED}
        options =
            new: true
            upsert: false
            sort: false
        ...
        Q().then ->
            Q.ninvoke @Model, 'findOneAndUpdate', query, updates, options
        .then (job) =>
            @callEndpoint job
        .fail (error) ->
    ...
    callEndpoint: (job) ->
        ...
        Q().then ->
            call options, (response) =>
                return Q response if @isValidResponse job, response
                return Q.reject response

    isValidResponse: (job, response) ->
